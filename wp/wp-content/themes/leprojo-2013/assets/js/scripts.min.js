/*! lasourisetlepiano-2013 - v1.0.0 - 2014-01-12
...
function removePhoneHref(a) {
    if ($("html").hasClass("no-touch")) {
        var b = $(a), c = '<span class="' + a.replace(".", "") + '">';
        c += b.html() + "</span>", b.parent().append(c), b.remove();
    }
}
...
function logArray(a) {
    $.each(a, function(a, b) {
        $.each(b, function(a, b) {
            console.log(b.spotName + " : " + b.coordinates);
        });
    });
}
...
function returnArray(a) {
    $.each(a, function(a, b) {
        $.each(b, function(a, b) {
            return b.spotName + " : " + b.coordinates;
        });
    });
}
...
function loadcss(a) {
    ...
    return c.type = "text/css", c.rel = "stylesheet", c.href = a, b.appendChild(c), 
    c;
}

function basementFitText(a, b, c) {
    if (document.getElementById(a)) {
        var d = $("#" + a);
        d.css("width", d.data("proportion")), $.getScript(jsFolder + "vendor/jquery.fittext.min.js", function() {
            $("#" + a).fitText(b, {
                minFontSize: c
            });
        });
...
function socialSharer() {
    function a(a) {
        window.prompt("Copier le lien dans le presse-papier : Ctrl+C, Enter", a);
    }
    document.getElementById("sharer-wrapper") && ($("#sharer-wrapper").hide(), $("#dislay-sharer-wrapper").click(function() {
        $("#dislay-sharer-wrapper").height($("#dislay-sharer-wrapper").height() + $("#dislay-sharer-wrapper").height() + $("#sharer-outer-wrapper").height() - 3);
    }, function() {
...
(function(a, b, c) {
    ...
    b.matchMedia = b.matchMedia || function(a) {
        ...
        response_obj: function(a, b) {
            for (var c = 0, d = b.length; d > c; c++) if ("function" == typeof b[c]) return b[c]({
        ...
        lib_methods: {
            ...
            scrollTo: function(c, d, e) {
                if (!(0 > e)) {
                    var f = d - a(b).scrollTop(), g = 10 * (f / e);
...
})(libFuncName, this, this.document), function(a, b, c) {
    "use strict";
    Foundation.libs.interchange = {
            ...
            directives: {
                replace: function(a, b) {
                    if (/IMG/.test(a[0].nodeName)) {
                        var c = a[0].src;
                        if (new RegExp(b, "i").test(c)) return;
                        return a[0].src = b, a.trigger("replace", [ a[0].src, c ]);
                    }
                }
            }
        },
        init: function(b, c, d) {
            return Foundation.inherit(this, "throttle"), "object" == typeof c && a.extend(!0, this.settings, c), 
            this.events(), this.images(), "string" != typeof c ? this.settings.init : this[c].call(this, d);
        },
        events: function() {
            var c = this;
            a(b).on("resize.fndtn.interchange", c.throttle(function() {
                c.resize.call(c);
            }, 50));
        },
        resize: function() {
            var b = this.cache;
            if (!this.images_loaded) return setTimeout(a.proxy(this.resize, this), 50), void 0;
            for (var c in b) if (b.hasOwnProperty(c)) {
                var d = this.results(c, b[c]);
                d && this.settings.directives[d.scenario[1]](d.el, d.scenario[0]);
            }
        },
        results: function(b, c) {
            var d = c.length;
            if (d > 0) for (var e = a('[data-uuid="' + b + '"]'), f = d - 1; f >= 0; f--) {
                var g, h = c[f][2];
                if (g = this.settings.named_queries.hasOwnProperty(h) ? matchMedia(this.settings.named_queries[h]) : matchMedia(h), 
                g.matches) return {
                    el: e,
                    scenario: c[f]
                };
        ...
        update_images: function() {
            var b = c.getElementsByTagName("img"), d = b.length, e = 0, f = "data-" + this.settings.load_attr;
            this.cached_images = [], this.images_loaded = !1;
            for (var g = d - 1; g >= 0; g--) this.loaded(a(b[g]), function(a) {
                if (e++, a) {
                    var b = a.getAttribute(f) || "";
                    b.length > 0 && this.cached_images.push(a);
                }
                e === d && (this.images_loaded = !0, this.enhance());
            }.bind(this));
            return "deferred";
        },
        loaded: function(a, b) {
        ...
        enhance: function() {
            for (var c = this.images().length, d = c - 1; d >= 0; d--) this._object(a(this.images()[d]));
